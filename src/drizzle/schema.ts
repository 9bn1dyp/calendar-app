...
const updatedAt = timestamp("updateAt").notNull().defaultNow().$onUpdate(() => new Date())
...
/* Every schedule has many availabilities*/ 
...
export const ScheduleAvailabilityTable = pgTable('schedule availability', {
    id: uuid('id').primaryKey().defaultRandom(),
    scheduleId: uuid('scheduleld')
    .notNull()
...
/* Each availability belongs to one schedule */ 
